...
### 钱包充值订单恢复与自动补账（2026-09-23）

#### 解决的问题

旧流程在钱包返回交易哈希后才创建链上充值记录。浏览器扩展、页面关闭、网络中断或钱包接口异常时，交易可能已经广播并到达平台收款地址，但前端没有成功回传哈希，后台没有订单可核验，导致用户已付款却未上账。

#### 新流程

1. 用户连接钱包并创建支付意图时，后端立即创建 `pending` 的 `TopUp` 订单和 `CryptoDepositIntent`，绑定用户、链、币种、付款地址、收款地址、美元金额、精确基础单位金额与一次性 challenge。
2. 用户完成钱包签名后，调用独立授权接口保存签名和授权时间；签名内容包含订单、链、Token、精确金额、付款/收款地址与过期时间，不能被复用于其他订单。
3. 前端取得交易哈希时提交哈希，后端独立核验链上交易；页面关闭、扩展异常或提交请求失败不会取消已授权订单。
4. 已授权但尚未取得哈希的订单，单例 worker 每约 15 秒扫描平台收款地址。只在链、币种、Token 合约、付款地址、收款地址、精确基础单位金额、交易时间和授权窗口全部一致，且候选订单唯一时，才绑定交易哈希并进入正常链上核验。
5. 同一交易哈希、订单和余额更新均在事务内做幂等控制；交易失败、金额不符、收款地址不符、重复哈希、匹配不唯一或 RPC 异常都不会自动多入账。RPC 异常保持 `pending` 并重试，明确失败才标记 `failed`。

#### 时间窗口与边界

- 未完成钱包授权：订单创建后最多 30 分钟，过期后不扫描、不上账。
- 完成授权但未取得哈希：从授权起最多 10 分钟尝试恢复匹配。
- 已取得或恢复交易哈希：从哈希提交/绑定起最多 30 分钟等待链上确认。
- 没有有效钱包签名时，后台不会按地址猜测并自动入账；即使发现疑似转账也保留为未匹配，避免把他人付款错记到用户余额。
- 适用于 EVM、TRON 和 Solana 的平台收款扫描；Token 和原生币都按意图中记录的精确基础单位核验，避免前后端报价漂移。

#### 数据与运维

- `crypto_deposit_intents` 增加订单号、基础单位金额、授权/哈希/重试/恢复截止时间等字段；新增 `crypto_chain_scan_cursors` 保存各链扫描游标。
- 订单创建、签名授权、哈希提交、自动找回、确认、失败和过期均写入用户日志，便于排查。
- 生产数据库结构初始化使用独立的非 prepared statement 连接，避免 PostgreSQL 在 schema 变更时出现 `cached plan must not change result type`；业务查询仍保持原有连接配置。
- 2026-09-23 首次发布中，worker 因上述 PostgreSQL 迁移问题未启动；修复后以蓝绿方式重新发布。生产 API 与 worker 运行 revision：`ed2d982`。
