...
func RunExprWithRequest(exprStr string, params TokenParams, request RequestInput) (float64, TraceResult, error) {
	return RunExprWithRequestAt(exprStr, params, request, time.Time{})
}

// RunExprWithRequestAt evaluates an expression using evaluatedAt as the
// immutable clock value exposed to hour/minute/weekday/month/day. A zero time
// preserves the legacy behaviour and resolves to the current time once at the
// start of evaluation.
func RunExprWithRequestAt(exprStr string, params TokenParams, request RequestInput, evaluatedAt time.Time) (float64, TraceResult, error) {
	prog, err := CompileFromCache(exprStr)
	if err != nil {
		return 0, TraceResult{}, err
	}
	return runProgram(prog, params, request, evaluatedAt)
}
...
func RunExprByHashWithRequest(exprStr, hash string, params TokenParams, request RequestInput) (float64, TraceResult, error) {
	return RunExprByHashWithRequestAt(exprStr, hash, params, request, time.Time{})
}

func RunExprByHashWithRequestAt(exprStr, hash string, params TokenParams, request RequestInput, evaluatedAt time.Time) (float64, TraceResult, error) {
...
func runProgram(prog *vm.Program, params TokenParams, request RequestInput, evaluatedAt time.Time) (float64, TraceResult, error) {
	trace := TraceResult{}
	...
	env := map[string]interface{}{
		...
		"hour":    func(tz string) int { return timeInZoneAt(evaluatedAt, tz).Hour() },
		"minute":  func(tz string) int { return timeInZoneAt(evaluatedAt, tz).Minute() },
		"weekday": func(tz string) int { return int(timeInZoneAt(evaluatedAt, tz).Weekday()) },
		"month":   func(tz string) int { return int(timeInZoneAt(evaluatedAt, tz).Month()) },
		"day":     func(tz string) int { return timeInZoneAt(evaluatedAt, tz).Day() },
		"max":     math.Max,
		"min":     math.Min,
		"abs":     math.Abs,
		"ceil":    math.Ceil,
...
func timeInZoneAt(evaluatedAt time.Time, tz string) time.Time {
