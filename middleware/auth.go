...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if len(allowIps) > 0 {
			...
			if ip == nil {
				abortWithOpenAiMessage(c, http.StatusForbidden, common.TranslateMessage(c, i18n.MsgAuthClientIPInvalid), types.ErrorCodeAccessDenied)
				return
			}
			if common.IsIpInCIDRList(ip, allowIps) == false {
				abortWithOpenAiMessage(c, http.StatusForbidden, common.TranslateMessage(c, i18n.MsgAuthClientIPForbidden), types.ErrorCodeAccessDenied)
				return
			}
		...
		if tokenGroup != "" {
			...
			if _, ok := service.GetUserUsableGroups(userGroup)[tokenGroup]; !ok {
				abortWithOpenAiMessage(c, http.StatusForbidden, common.TranslateMessage(c, i18n.MsgAuthGroupForbidden, map[string]any{"Group": tokenGroup}), types.ErrorCodeAccessDenied)
				return
			}
			// check group in common.GroupRatio
			if !ratio_setting.ContainsGroupRatio(tokenGroup) {
				if tokenGroup != "auto" {
					abortWithOpenAiMessage(c, http.StatusForbidden, common.TranslateMessage(c, i18n.MsgAuthGroupDeprecated, map[string]any{"Group": tokenGroup}), types.ErrorCodeAccessDenied)
					return
				}
			}
...
func SetupContextForToken(c *gin.Context, token *model.Token, parts ...string) error {
	...
	if len(parts) > 1 {
		if model.IsAdmin(token.UserId) {
			...
			message := common.TranslateMessage(c, i18n.MsgAuthSpecificChannelForbidden)
