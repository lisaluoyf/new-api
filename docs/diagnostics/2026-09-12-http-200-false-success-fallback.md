...
卡片不展示用户邮箱、用户名或 user ID。当前卡片开头固定展示请求 ID、当前渠道和模型，便于在飞书和数据库中检索对应请求；后面展示上游诊断信息和 retry decision。渠道格式为 `#渠道ID/渠道名`，例如 `#219/lingsu-gpt-pro`。竞速、最终失败、官方兜底和客户端断开通知仍发送到原来的日志群，只有假成功 feedback 发送到专用群。
...
本轮使用生产日志库中的结构化字段，而不是飞书消息数量。统计起点是第一版假成功识别代码实际产生的第一条日志：北京时间 `2026-09-12 12:18:51`。统计截止到本轮查询时（约北京时间 `16:01`）。以 `request_id` 去重；同一请求在多个渠道失败会产生多条错误日志，不能直接把错误日志行数当作请求数。

### 8.2 结果

| 指标 | 数量 | 说明 |
|---|---:|---|
| 假成功诊断事件 | 107 条 | 同一个请求可能命中多个渠道，因此会重复 |
| 涉及独立请求 | 93 个 | 按 `request_id` 去重 |
| 实际切换到第二渠道 | 88 个 | 日志中的 `use_channel` 链路长度为 2 |
| fallback 后最终成功 | 1 个 | 后续出现消费日志（`type=2`） |
| 切到第二渠道后仍失败 | 87 个 | 第二渠道没有完成请求 |
| 第一渠道即被阻止继续切换 | 5 个 | 最终重试原因为 `stream_already_started` |

因此，当时重试次数为 1 的真实流程是：

```text
93 个独立假成功请求
├─ 5 个：第一渠道发现异常，但流已被判定开始，无法继续切换
└─ 88 个：确实切到第二渠道
   ├─ 1 个：第二渠道成功
   └─ 87 个：第二渠道也失败
```

这说明多渠道 fallback 已经实际执行，低成功率的主要原因不是完全没有切换，而是第二渠道也在同一时间段遇到容量、并发或响应异常，且当时最多只尝试两个渠道。
...
第二渠道失败后的最终重试原因主要是 `stream_already_started`，其次是 `retry_times_exhausted`。这意味着第二渠道发生异常时，系统通常已经向客户端提交了该渠道的流，不能再透明切换第三渠道；当时重试次数为 1，也没有第三次尝试机会。

### 8.4 重试次数调整后的观察方法

本轮之后将重试次数从 1 调整为 2。新请求最多可以经历：
...
后续应单独统计调整后的时间窗口，重点查看：

1. `use_channel` 是否出现长度为 3 的链路。
2. 第二渠道失败后，是否成功进入第三渠道。
3. 第三渠道是否成功，还是三个渠道都返回容量/并发错误。
4. `stream_already_started` 是否仍在第一或第二渠道阻止继续 fallback。

不能把调整前的 93 个请求与调整后的请求混合计算成功率。若第三渠道仍失败，需要进一步确认候选渠道是否共享同一上游账号、节点或官方容量池；若仍大量停在 `stream_already_started`，则应优先修复“HTTP writer 已写入”和“有效模型输出已提交”混为一谈的问题。

### 8.5 数据解读限制

- `107 条诊断事件`不是 `107 个请求`；一次请求可能在多个渠道分别命中假成功。
- 旧版历史日志中可能存在缺少 `request_id` 或缺少 `admin_info.upstream_false_success` 的记录，不纳入本轮精确关联。
- “最终成功”以同一 `request_id` 出现消费日志（`type=2`）为准；只有错误日志或退款日志不能算成功。
- `action=discard_channel_and_fallback` 只表示识别器要求执行 fallback，是否实际重试还要结合 `retry_decision.should_retry`、`retry_decision.reason` 和 `use_channel` 链路判断。
...
- `81d1dcc`：假成功通知增加请求 ID。
- `5b678b6`：假成功通知增加渠道和模型，渠道格式为 `#ID/名称`。
- `419ad85`：移除假成功通知中的反引号代码样式，保证飞书可搜索。
- APIMaster 总仓库指针提交：`3442fea`（指向 NewAPI `419ad85`）。
