...
type FreeModelMember struct {
	...
	DailyRequestLimitGroup string `json:"daily_request_limit_group" gorm:"type:varchar(128);not null;default:''"`
...
func UpsertFreeModelMember(member FreeModelMember) error {
	return DB.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"enabled", "priority", "weight", "text", "vision", "tools", "codex_tools", "codex_priority", "codex_weight", "required_tool_call", "json_object", "json_schema", "chat_completions", "responses", "messages", "max_context_tokens", "timeout_ms", "daily_request_limit", "daily_request_limit_group", "updated_at"}),
	}).Create(&member).Error
}
