...
export class SecureVerificationService {
  ...
  static async verify2FA(code) {
    ...
    if (!verifyResponse.data?.success) {
      throw new Error(
        verifyResponse.data?.message || i18n.t('验证失败，请重试'),
      );
    }
  ...
  static async verifyPasskey() {
    try {
      ...
      if (!beginResponse.data?.success) {
        throw new Error(
          beginResponse.data?.message || i18n.t('验证失败，请重试'),
        );
      }
      ...
      if (!finishResponse.data?.success) {
        throw new Error(
          finishResponse.data?.message || i18n.t('验证失败，请重试'),
        );
      }
      ...
      if (error.name === 'NotAllowedError') {
        ...
        throw new Error('Passkey verification state is invalid');
      } else {
        throw error;
      }
    }
