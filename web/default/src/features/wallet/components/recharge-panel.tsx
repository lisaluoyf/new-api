...
export function RechargePanel({
  ...
  const { processing: pancakeLoading, processWaffoPancakePayment } =
    useWaffoPancakePayment()
  const { processing: plategaLoading, processPlategaPayment } =
    usePlategaPayment()
  ...
  const [minTopupDialog, setMinTopupDialog] = useState<{ min: number } | null>(
    null
  )

  const requestAmount = customAmount
    ? parseFloat(customAmount) || 0
    : selectedAmount
  const amountDiscount =
    requestAmount > 0 ? (topupInfo?.discount?.[requestAmount] ?? 1) : 1
  const promoDiscount =
    promoInfo && requestAmount === promoInfo.amount ? promoInfo.discount : 1
  ...
  useEffect(() => {
    ...
    function tick() {
      const secs = Math.max(
        0,
        promoInfo!.expires_at - Math.floor(Date.now() / 1000)
      )
      if (secs === 0) {
        setCountdown('')
        return
  ...
  async function handleEpayPay(method: string) {
    ...
    try {
      ...
      if (isApiSuccess(res) && res.url) {
        ...
        presentPaymentError(
          typeof res.data === 'string' ? res.data : res.message
        )
      }
    } catch {
      toast.error(paymentErrorMessage())
            ...
            <div className='grid grid-cols-3 gap-2'>
              {PRESET_AMOUNTS.map((amount) => {
                ...
                return (
                  ...
                  >
                    {isPromo && (
                      <span className='absolute -top-2.5 -right-1.5 flex flex-col items-center rounded-md bg-amber-500 px-1.5 py-0.5 text-white shadow'>
                        <span className='text-[7px] leading-tight font-semibold tracking-wide uppercase opacity-90'>
                          {t('New user')}
                        </span>
                    ...
                    {!isPromo && hasAmountDiscount && (
                      <span className='absolute -top-2 -right-1.5 flex items-center rounded-md bg-orange-700 px-1.5 py-0.5 text-[10px] leading-tight font-extrabold text-white shadow'>
                        {amountDiscountLabel}
                      </span>
                    )}
                    ${amount}
                    {isPromo && (
                      <div className='mt-0.5 text-[10px] font-normal text-amber-600'>
                        {t('pay ${{pay}} → get ${{amount}}', {
                          pay: Number(promoInfo.pay_amount || 0).toFixed(2),
            ...
            {promoInfo && countdown && (
              <div className='mt-2 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs'>
                <span className='font-semibold text-amber-700'>
                  {t('🎁 New user exclusive · {{pct}}% off first top-up', {
                    pct: Math.round((1 - promoInfo.discount) * 100),
                  })}
                </span>
          ...
          <div>
            ...
            <div className='relative'>
              ...
              <input
                ...
                placeholder='0.00'
                className='border-input bg-background text-foreground placeholder:text-muted-foreground w-full rounded-lg border py-2 pr-3 pl-7 text-sm outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-200'
              />
            </div>
          </div>
            ...
            {hasRechargeChannels ? (
              <div className='grid grid-cols-2 gap-2 sm:grid-cols-3'>
                      ...
                      <TooltipContent className='max-w-[260px]'>
                        <div className='space-y-1.5'>
                          <div className='text-[11px] font-semibold'>
                            {t('Global cards and local methods')}
                          </div>
                          <div className='flex flex-col gap-1'>
                            {CLINK_LOCAL_METHODS.map((m) => (
                              ...
                              >
                                <span className='inline-flex w-6 shrink-0 justify-center rounded bg-white/20 px-1 py-px font-mono text-[10px]'>
                                  {m.code}
                                </span>
                ...
                <button
                  type='button'
                  disabled
                  className='border-border flex cursor-not-allowed items-center gap-3 rounded-xl border bg-white px-3 py-3 text-left opacity-40'
                >
      ...
      >
        <DialogContent className='max-w-sm text-center' showCloseButton={false}>
          ...
          <div className='mt-2 flex justify-center'>
            <Button
              className='min-w-24'
              onClick={() => setMinTopupDialog(null)}
            >
              {t('OK')}
            </Button>
          </div>
