...
func GeminiChatStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	usage, err := geminiStreamHandler(c, info, resp, func(data string, geminiResponse *dto.GeminiChatResponse) bool {
		...
		response.Model = service.FreeModelResponseName(info.OriginModelName, info.UpstreamModelName)
		...
		if info.SendResponseCount == 0 {
			// send first response
			emptyResponse := helper.GenerateStartEmptyResponse(id, createAt, service.FreeModelResponseName(info.OriginModelName, info.UpstreamModelName), nil)
		...
		if isStop {
			_ = handleStream(c, info, helper.GenerateStopResponse(id, createAt, service.FreeModelResponseName(info.OriginModelName, info.UpstreamModelName), finishReason))
		}
		return true
	})
	...
	response := helper.GenerateFinalUsageResponse(id, createAt, service.FreeModelResponseName(info.OriginModelName, info.UpstreamModelName), *usage)
...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	fullTextResponse.Model = service.FreeModelResponseName(info.OriginModelName, info.UpstreamModelName)
	usage := buildUsageFromGeminiMetadata(geminiResponse.UsageMetadata, info.GetEstimatePromptTokens())
