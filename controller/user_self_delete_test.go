...
func TestDeleteSelfRevokesOldSessionsAndInternalLogin(t *testing.T) {
	...
	r.GET("/token-or-session", middleware.TokenOrUserAuth(), func(c *gin.Context) { c.Status(http.StatusNoContent) })
	r.GET("/optional-session", middleware.TryUserAuth(), func(c *gin.Context) { c.JSON(http.StatusOK, gin.H{"authenticated": c.GetInt("id") > 0}) })
	...
	w = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/token-or-session", nil)
	request.AddCookie(oldCookie)
	r.ServeHTTP(w, request)
	require.Equal(t, http.StatusUnauthorized, w.Code)
	w = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/optional-session", nil)
	request.AddCookie(oldCookie)
	r.ServeHTTP(w, request)
