...
// clientgone fallback（竞速 fallback）：
// 流式请求按首个有效数据帧判胜；非流式文本请求按完整成功响应判胜。
...
const defaultNonStreamHedgeMaxResponseMB = 8
const nonStreamHedgeCompleteResponseMSKey = "non_stream_hedge_complete_response_ms"
const nonStreamHedgeWinnerRoleKey = "non_stream_hedge_winner_role"
const nonStreamHedgeWinnerChannelIDKey = "non_stream_hedge_winner_channel_id"
const nonStreamHedgeWinnerChannelNameKey = "non_stream_hedge_winner_channel_name"
...
func shouldClientGoneHedge(c *gin.Context, info *relaycommon.RelayInfo, relayFormat types.RelayFormat, retryIndex int) (model_setting.ClientGoneFallbackPolicy, bool) {
	...
	if info.IsPlayground || info.IsChannelTest {
		return model_setting.ClientGoneFallbackPolicy{}, false
	}
	if !isHedgeSupportedTextRequest(info, relayFormat) {
		return model_setting.ClientGoneFallbackPolicy{}, false
	}
	if ratio_setting.ContainsAudioRatio(info.OriginModelName) || ratio_setting.ContainsAudioCompletionRatio(info.OriginModelName) {
		return model_setting.ClientGoneFallbackPolicy{}, false
	}
	if _, ok := c.Get("specific_channel_id"); ok {
		return model_setting.ClientGoneFallbackPolicy{}, false
	}
	policy, ok := model_setting.FindClientGoneFallbackPolicy(info.OriginModelName)
	if !ok || !policy.AppliesToStream(info.IsStream) {
		return model_setting.ClientGoneFallbackPolicy{}, false
	}
	return policy, true
}

func isHedgeSupportedTextRequest(info *relaycommon.RelayInfo, relayFormat types.RelayFormat) bool {
	switch relayFormat {
...
type hedgeAttempt struct {
	...
	buffer     *helper.BufferedResponseWriter
...
func (a *hedgeAttempt) completeLabel() string {
	if a == nil || a.finishedAt.IsZero() || a.info == nil {
		return "未完成"
	}
	return fmt.Sprintf("%.1fs", a.finishedAt.Sub(a.info.StartTime).Seconds())
}

func hedgeModeLabel(info *relaycommon.RelayInfo) string {
...
func runClientGoneHedgedRelay(c *gin.Context, relayInfo *relaycommon.RelayInfo, relayFormat types.RelayFormat,
	...
	if !relayInfo.IsStream {
		return runNonStreamHedgedRelay(c, relayInfo, relayFormat, primaryChannel, retryParam, policy)
	}
	...
	case <-timer.C:
		...
		if !alreadyWon && c.Request.Context().Err() == nil {
			preparedHedge := prepareHedgeAttempt(c, relayInfo, retryParam, primaryChannel, buildAttempt)
			if preparedHedge != nil {
				raceMu.Lock()
				if winner != nil {
					raceMu.Unlock()
					preparedHedge.rollback()
					break
				}
				hedge = preparedHedge.attempt
				attempts = append(attempts, hedge)
				raceMu.Unlock()
				preparedHedge.commit()
				syncHedgeUseChannels(c, primary, hedge)
				startAttempt(hedge)
			}
		}
	}
	...
	if loser != nil {
		...
		gopool.Go(func() {
			...
			service.RecordHedgeLoserConsumption(loserAttempt.c, loserAttempt.info,
				loserAttempt.channel.Id, loserAttempt.channel.Name, winnerChannelId, finalWinner.channel.Name, loserAttempt.err)
		})
	}
...
func runNonStreamHedgedRelay(c *gin.Context, relayInfo *relaycommon.RelayInfo, relayFormat types.RelayFormat,
	primaryChannel *model.Channel, retryParam *service.RetryParam, policy model_setting.ClientGoneFallbackPolicy) *types.NewAPIError {
	...
	thresholdSeconds := policy.FirstByteTimeoutSeconds(int64(len(bodyBytes)))
	threshold := time.Duration(thresholdSeconds) * time.Second
	maxBytes := common.GetEnvOrDefault("NON_STREAM_HEDGE_MAX_RESPONSE_MB", defaultNonStreamHedgeMaxResponseMB) * 1024 * 1024
	...
	declareWinner := func(a *hedgeAttempt) bool {
		if a == nil || a.err != nil {
			return false
		}
		if a.buffer != nil && a.buffer.Overflowed() {
			a.err = types.NewError(fmt.Errorf("non-stream hedge response exceeds buffer limit"), types.ErrorCodeBadResponse)
			return false
		}
		if !hasBillableHedgeUsage(a) {
			a.err = service.NewUpstreamNoUsageError()
			return false
		}
		raceMu.Lock()
		defer raceMu.Unlock()
	...
	buildAttempt := func(role string, channel *model.Channel) (*hedgeAttempt, error) {
		...
		cc.Set(common.KeyBodyStorage, storage)
		cc.Request.Body = io.NopCloser(storage)
		a.buffer = helper.NewBufferedResponseWriter(maxBytes)
		cc.Writer = a.buffer
		a.c = cc
	...
	startAttempt := func(a *hedgeAttempt) {
		gopool.Go(func() {
			defer func() {
				if r := recover(); r != nil {
					logger.LogError(a.c, fmt.Sprintf("non-stream hedge %s attempt panic: %v", a.role, r))
					a.err = types.NewError(fmt.Errorf("hedge attempt panic: %v", r), types.ErrorCodeDoRequestFailed)
				}
				if a.buffer != nil && a.buffer.Overflowed() && a.err == nil {
					a.err = types.NewError(fmt.Errorf("non-stream hedge response exceeds buffer limit"), types.ErrorCodeBadResponse)
				}
				a.finishedAt = time.Now()
				if a.err == nil {
	...
	primary, buildErr := buildAttempt(relaycommon.HedgeRolePrimary, primaryChannel)
	if buildErr != nil {
		logger.LogError(c, fmt.Sprintf("non-stream hedge: build primary attempt failed, fallback to normal relay: %s", buildErr.Error()))
		return dispatchHedgeRelay(c, relayInfo, relayFormat)
	}
	raceMu.Lock()
	attempts = append(attempts, primary)
	...
	finalWinner := awaitNonStreamRaceOutcome(winnerCh, primary, hedge)
	hedgeArmed := hedge != nil || c.GetBool("clientgone_hedge_triggered")
	...
	if err := helper.FlushBufferedResponse(c.Writer, finalWinner.buffer); err != nil {
		finalWinner.err = types.NewError(err, types.ErrorCodeBadResponse)
		return finalWinner.err
	}
	recordNonStreamHedgeWinnerInfo(finalWinner)
	winnerUsage := service.FinalizeHedgeWinnerBilling(finalWinner.c, finalWinner.info)
	if loser != nil {
		loserAttempt := loser
		winnerChannelId := finalWinner.channel.Id
		winnerChannelName := finalWinner.channel.Name
		requestID := c.GetString(common.RequestIdKey)
		gopool.Go(func() {
			if !waitAttempt(loserAttempt, 120*time.Second) {
				logger.LogError(context.Background(), fmt.Sprintf(
					"non-stream hedge: loser attempt (channel #%d) did not finish within grace period, skip loser accounting, request_id=%s",
					loserAttempt.channel.Id, requestID))
				return
			}
			service.RecordHedgeLoserConsumption(loserAttempt.c, loserAttempt.info,
				loserAttempt.channel.Id, loserAttempt.channel.Name, winnerChannelId, winnerChannelName, loserAttempt.err)
		})
	}
	...
	if hedgeArmed {
		notifyClientGoneHedgeResultWithUsage(c, relayInfo, thresholdSeconds, len(bodyBytes), primary, hedge, finalWinner, nil, winnerUsage)
	}
	return nil
}

type preparedHedgeAttempt struct {
	attempt  *hedgeAttempt
	commit   func()
	rollback func()
}

// prepareHedgeAttempt 只负责选渠、建 attempt、准备上下文和预占计费，不启动请求。
// 非流式竞速需要先把 attempt 放入 attempts 列表再启动，避免极快返回时漏标 loser。
func prepareHedgeAttempt(c *gin.Context, relayInfo *relaycommon.RelayInfo, retryParam *service.RetryParam,
	primaryChannel *model.Channel, buildAttempt func(string, *model.Channel) (*hedgeAttempt, error)) *preparedHedgeAttempt {

	retrySnapshot := retryParam.Snapshot()
	routeSnapshot := snapshotHedgeRouteContext(c)
	var hedge *hedgeAttempt
	rollback := func() {
		retryParam.Restore(retrySnapshot)
		routeSnapshot.restore(c)
		if hedge != nil && hedge.cancel != nil {
			hedge.cancel()
		}
	}
	...
	commit := func() {
		// hedge 准备完成且竞速仍未分出赢家时，才算真正触发。
		c.Set("clientgone_hedge_triggered", true)
		addUsedChannel(c, channel.Id)
		triggerReason := "no first byte"
		if relayInfo != nil && !relayInfo.IsStream {
			triggerReason = "no complete response"
		}
		logger.LogInfo(c, fmt.Sprintf("clientgone hedge triggered: primary #%d %s, racing with #%d/%s",
			primaryChannel.Id, triggerReason, channel.Id, channel.Name))
	}

	return &preparedHedgeAttempt{
		attempt:  hedge,
		commit:   commit,
		rollback: rollback,
	}
...
func syncHedgeUseChannels(source *gin.Context, attempts ...*hedgeAttempt) {
	if source == nil {
		return
	}
	useChannels := append([]string(nil), source.GetStringSlice("use_channel")...)
	if len(useChannels) == 0 {
		return
	}
	for _, attempt := range attempts {
		if attempt == nil || attempt.c == nil {
			continue
		}
		attempt.c.Set("use_channel", append([]string(nil), useChannels...))
	}
}
...
func awaitNonStreamRaceOutcome(winnerCh <-chan *hedgeAttempt, primary, hedge *hedgeAttempt) *hedgeAttempt {
	primaryDone := primary.done
	var hedgeDone chan struct{}
	if hedge != nil {
		hedgeDone = hedge.done
...
func hasBillableHedgeUsage(a *hedgeAttempt) bool {
	if a == nil || a.info == nil || a.info.HedgeState == nil {
		return false
	}
	usage, ok := a.info.HedgeState.PeekDeferredUsage()
	if !ok || usage == nil {
		return false
	}
	return billableTextUsageTokens(usage) > 0
}

func billableTextUsageTokens(usage *dto.Usage) int {
	if usage == nil {
		return 0
	}
	total := usage.TotalTokens + usage.PromptTokens + usage.CompletionTokens + usage.InputTokens + usage.OutputTokens
	...
	total += usage.PromptTokensDetails.AudioTokens
	total += usage.PromptTokensDetails.ImageTokens
	if usage.InputTokensDetails != nil {
		total += usage.InputTokensDetails.CachedTokens
		total += usage.InputTokensDetails.CachedCreationTokens
		total += usage.InputTokensDetails.TextTokens
		total += usage.InputTokensDetails.AudioTokens
		total += usage.InputTokensDetails.ImageTokens
	}
	total += usage.CompletionTokenDetails.ReasoningTokens
	total += usage.CompletionTokenDetails.AudioTokens
	total += usage.ClaudeCacheCreation5mTokens + usage.ClaudeCacheCreation1hTokens
	return total
}

func recordNonStreamHedgeWinnerInfo(winner *hedgeAttempt) {
	if winner == nil || winner.c == nil || winner.channel == nil || winner.info == nil || winner.finishedAt.IsZero() {
		return
	}
	winner.c.Set(nonStreamHedgeCompleteResponseMSKey, winner.finishedAt.Sub(winner.info.StartTime).Milliseconds())
	winner.c.Set(nonStreamHedgeWinnerRoleKey, winner.role)
	winner.c.Set(nonStreamHedgeWinnerChannelIDKey, winner.channel.Id)
	winner.c.Set(nonStreamHedgeWinnerChannelNameKey, winner.channel.Name)
}

type hedgeRouteContextSnapshot struct {
	values map[constant.ContextKey]any
	exists map[constant.ContextKey]bool
}

func snapshotHedgeRouteContext(c *gin.Context) hedgeRouteContextSnapshot {
	...
	snapshot := hedgeRouteContextSnapshot{
		values: make(map[constant.ContextKey]any, len(keys)),
		exists: make(map[constant.ContextKey]bool, len(keys)),
	}
	for _, key := range keys {
		value, ok := common.GetContextKey(c, key)
		snapshot.values[key] = value
		snapshot.exists[key] = ok
	}
	return snapshot
}

func (s hedgeRouteContextSnapshot) restore(c *gin.Context) {
	if c == nil {
		return
	}
	for key, existed := range s.exists {
		keyString := string(key)
		if existed {
			c.Set(keyString, s.values[key])
			continue
		}
		if c.Keys != nil {
...
func notifyClientGoneHedgeResultWithUsage(c *gin.Context, relayInfo *relaycommon.RelayInfo, thresholdSeconds int, bodySize int,
	...
	lines := []string{
		...
		fmt.Sprintf("- 模式：`%s`", hedgeModeLabel(relayInfo)),
		fmt.Sprintf("- body：`%.2f MB`", float64(bodySize)/1024/1024),
		fmt.Sprintf("- 触发：主渠道 `%s` 超过阈值 `%ds` %s", primary.channelLabel(), thresholdSeconds, hedgeTriggerText(relayInfo)),
		fmt.Sprintf("- 主渠道 %s：<font color='red'>**%s**</font>", hedgeMetricName(relayInfo), hedgeMetricLabel(relayInfo, primary)),
	}
	if hedge != nil {
		lines = append(lines,
			fmt.Sprintf("- hedge 渠道：`%s`（于 `+%.1fs` 启动）", hedge.channelLabel(), hedge.startedAt.Sub(relayInfo.StartTime).Seconds()),
			fmt.Sprintf("- hedge %s：<font color='green'>**%s**</font>", hedgeMetricName(relayInfo), hedgeMetricLabel(relayInfo, hedge)),
		)
	} else {
	...
	if winner != nil {
		...
		lines = append(lines,
			fmt.Sprintf("- 判胜：`%s（%s）`", winner.role, winner.channelLabel()),
			fmt.Sprintf("- 响应完成：`%s` 于 `%.1fs` 完成，total tokens `%s`", winner.channelLabel(), completedIn, tokens),
		)
		if loser := pickLoser(primary, hedge, winner); loser != nil {
			lines = append(lines, fmt.Sprintf("- 败者：`%s` 已取消（%s `%s`，收到 chunk `%d`）",
				loser.channelLabel(), hedgeMetricName(relayInfo), hedgeMetricLabel(relayInfo, loser), loser.info.ReceivedResponseCount))
		}
	}

	title := "NewAPI 竞速 fallback"
	if finalErr != nil {
		title += "（最终失败）"
...
func hedgeTriggerText(info *relaycommon.RelayInfo) string {
	if info != nil && !info.IsStream {
		return "未完整成功返回"
	}
	return "未出首字"
...
func hedgeMetricName(info *relaycommon.RelayInfo) string {
	if info != nil && !info.IsStream {
		return "完整响应耗时"
	}
	return "frt"
...
func hedgeMetricLabel(info *relaycommon.RelayInfo, attempt *hedgeAttempt) string {
	if info != nil && !info.IsStream {
		return attempt.completeLabel()
	}
	return attempt.frtLabel()
}
