...
func EnableChannelModel(channelId int, modelName string, channelName string, expectedVersions ...string) {
	modelName = strings.TrimSpace(modelName)
	if channelId <= 0 || modelName == "" {
		return
	}

	changed, err := setAutomaticModelStatus(channelId, modelName, "Recovery probe passed", true, "recovery_probe", expectedVersions...)
	if err != nil {
		common.SysError(fmt.Sprintf("failed to enable model atomically: channel_id=%d model=%s error=%v", channelId, modelName, err))
		return
	}
	if !changed {
		return
